...
export default function CompanyDashboard() {
  ...
  const monthlyRevenueData = [/* can be derived from transactions */];
      ...
      <div style={pageWrapper}>
        ...
        <main style={isMobile ? mobileMain : mainContent}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px,1fr))', gap: 16, marginBottom: 32 }}>
            <div style={card}><FiDollarSign color="#c026d3"/><h2>${revenue.toFixed(2)}</h2><p>Revenue this month</p></div>
            <div style={card}><FiTrendingDown color="#ef4444"/><h2>${totalExpenses.toFixed(2)}</h2><p>Expenses this month</p></div>
            <div style={card}><FiTrendingUp color={netProfit>=0?'#22c55e':'#ef4444'}/><h2>${netProfit.toFixed(2)}</h2><p>Net Profit</p></div>
            <div style={card}><FiClock color="#f59e0b"/><h2>{pendingCount}</h2><p>Pending Transactions</p></div>
          </div>

          {/* Charts placeholder */}
          <div style={card}><ReportCharts transactions={transactions} /></div>
          ...
          <div style={card}>
            {monthlyTx.slice(0,5).map(tx => (
              <div key={tx.id} style={{ display:'flex', justifyContent:'space-between', padding: '8px 0', borderBottom:'1px solid rgba(255,255,255,0.1)' }}>
                <span>{tx.description}</span><span>${parseFloat(tx.amount||0).toFixed(2)}</span>
              </div>
            ))}
          </div>
          ...
          <div style={card}>
            {expenses.slice(0,3).map(e => (
              <div key={e.id} style={{ display:'flex', justifyContent:'space-between', padding: '8px 0' }}>
                <span>{e.staffName}</span><span>${e.amount}</span>
              </div>
            ))}
          </div>
