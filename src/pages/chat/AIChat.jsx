...
import { FiCpu, FiSend, FiTrash2, FiMessageSquare } from 'react-icons/fi';
...
export default function AIChat() {
  ...
  useEffect(() => {
    if (!companyId) return;
    const q = query(collection(db, `chats/${companyId}/ai/messages`), orderBy('createdAt', 'asc'));
  ...
  const sendMessage = async (text) => {
    ...
    // Save user message and get company context
    await addDoc(collection(db, `chats/${companyId}/ai/messages`), {
      text: question, sender: 'user', senderName: currentUser.displayName || 'You',
      createdAt: serverTimestamp(),
    });

    try {
      const companyContext = { companyId }; // In real app fetch actual financial data
      ...
      setMessages(prev => [...prev, { text: 'Sorry, I encountered an error. Please try again.', sender: 'ai', createdAt: new Date() }]);
    } finally {
      setLoading(false);
    }
  };
          ...
          <div style={messagesContainer}>
            {messages.length === 0 && (
              <div style={{ textAlign: 'center', color: 'rgba(255,255,255,0.6)', marginTop: 40 }}>
                <p>Hello! I'm your Acciox AI assistant. I have access to your company's financial data and can answer questions about transactions, expenses, reports and more.</p>
                <div style={{ display: 'flex', gap: 8, justifyContent: 'center', marginTop: 20, flexWrap: 'wrap' }}>
                  {suggestedQuestions.map((q, i) => (
                    <button key={i} onClick={() => sendMessage(q)} style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 20, padding: '8px 16px', color: '#fff', cursor: 'pointer', fontSize: '0.85rem' }}>{q}</button>
                  ))}
                </div>
              </div>
            )}
            {messages.map((msg, idx) => (
              <div key={idx} style={{ display: 'flex', flexDirection: 'column', alignItems: msg.sender === 'user' ? 'flex-end' : 'flex-start', gap: 2 }}>
                {msg.sender !== 'user' && <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginLeft: 8 }}>
                  <div style={{ width: 24, height: 24, borderRadius: '50%', background: 'linear-gradient(135deg, #c026d3, #e879f9)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><FiCpu size={14} color="#fff" /></div>
                  <span style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.6)' }}>Acciox AI</span>
                </div>}
                <div style={bubble(msg.sender === 'user')}>{msg.text}</div>
              </div>
            ))}
            {loading && (
              <div style={{ alignSelf: 'flex-start', background: 'rgba(255,255,255,0.1)', borderRadius: 16, padding: '10px 16px', display: 'flex', gap: 4 }}>
                <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#c026d3', animation: 'dotPulse 1.4s infinite' }} />
                <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#c026d3', animation: 'dotPulse 1.4s infinite 0.2s' }} />
                <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#c026d3', animation: 'dotPulse 1.4s infinite 0.4s' }} />
              </div>
            )}
